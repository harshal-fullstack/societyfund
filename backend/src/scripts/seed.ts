...
export async function seedDatabase() {
  ...
  await dataStore.saveCategories(categories);
  await dataStore.saveReserveFunds(reserveFunds);
  await dataStore.saveTransactions(transactions);
  await dataStore.saveInvoices(invoices);
  await dataStore.saveAuditLogs(auditLogs);
  for (const u of users) {
    const existing = await dataStore.findUserByEmail(u.email);
    if (!existing) {
      await dataStore.createUser(u);
    }
  }

  // Save new entity collections
  for (const n of notices) {
    await dataStore.createNotice(n);
  }
  for (const fy of financialYears) {
    await dataStore.createFinancialYear(fy);
  }
  ...
  for (const kpi of kpiSnapshots) {
    const snapshots = await dataStore.getKpiSnapshots();
    if (!snapshots.find(s => s._id === kpi._id)) {
      await dataStore.captureKpiSnapshot(); // Use live capture for latest month
    }
  }

  for (const rpt of reportHistory) {
    await dataStore.createReportEntry(rpt);
  }

  for (const exp of exportHistory) {
    await dataStore.createExportEntry(exp);
  }
